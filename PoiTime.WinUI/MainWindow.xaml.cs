...
namespace PoiTime.WinUI
{
    public sealed partial class MainWindow : Window
    {
        ...
        // ================= 开机自启逻辑 (现代 WinUI 3 方案) =================

        private async void RootGrid_Loaded(object sender, RoutedEventArgs e)
        {
            try
            {
                // 获取我们在 appxmanifest 里填写的那个 TaskId
                StartupTask startupTask = await StartupTask.GetAsync("PoiTimeAutoStart");

                // 如果状态是 Disabled，说明还没开启自启
                if (startupTask.State == StartupTaskState.Disabled)
                {
                    var dialog = new ContentDialog
                    {
                        Title = "开机启动设置",
                        Content = "检测到尚未设置开机启动。为了确保舰娘能在后台按时为你报时，是否现在开启开机自启？",
                    ...
                    {
                        // 呼出系统级的权限请求，Windows 会自己处理好一切
                        StartupTaskState newState = await startupTask.RequestEnableAsync();

                        // 可选：检查用户是否在系统弹窗里点了拒绝
                        if (newState == StartupTaskState.DisabledByUser || newState == StartupTaskState.Disabled)
                        {
                            var failDialog = new ContentDialog
                            {
                                Title = "提示",
                                Content = "开机自启开启失败或被拒绝。你也可以稍后在任务管理器的「启动」选项卡中手动开启。",
                                CloseButtonText = "知道了",
                                XamlRoot = this.Content!.XamlRoot
                            };
            ...
            {
                // 如果在非打包模式下调试运行，或者遇到异常，会跳到这里
                System.Diagnostics.Debug.WriteLine($"请求自启权限失败: {ex.Message}");
            }
        }
